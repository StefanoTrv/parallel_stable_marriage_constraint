...
def main():
    ...
    #Compiles solver
    print("Compiling solver with benchmarking options...\n")
    subprocess.run("cmake -S ../fzn-minicpp/ -B ../fzn-minicpp/build/ -DCMAKE_BUILD_TYPE=Release -DBENCHMARK_STC_PROPAGATIONS=ON", shell=True, text=True)
    subprocess.run("make -C ../fzn-minicpp/build/", shell=True, text=True)
    ...
    #Compiles solver
    print("Restoring solver (re-compiling)...\n")
    subprocess.run("cmake -S ../fzn-minicpp/ -B ../fzn-minicpp/build/ -DCMAKE_BUILD_TYPE=Release", shell=True, text=True)
    subprocess.run("make -C ../fzn-minicpp/build/", shell=True, text=True)
    print("Done\n")
