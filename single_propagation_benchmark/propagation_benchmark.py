...
def main():
    ...
    empty_output_folder()
    #line format: <instance_size> <number_of_tests> <force_order_of_binding> <double_constraint>
    i = 0
    for l in config:
        l = l.split("\n")[0]
        n :int = int(l.split(" ")[0])
        ...
        print("Beginning propagation benchmark with "+str(number_of_tests)+" tests of size n="+str(n)+", with force_order="+str(force_order)+", double_constraint="+str(double_constraint))
        run_tests(n,number_of_tests,force_order, double_constraint, output_file_serial, output_file_parallel)
        print("Completed "+str(number_of_tests)+" tests.\n")
        output_file_serial.close()
        output_file_parallel.close()
        i = i + 1
    subprocess.run("rm serial_input.ozn", shell=True, text=True)
    subprocess.run("rm parallel_input.ozn", shell=True, text=True)
    print("Completed all tests.")

def run_tests(n : int, number_of_tests : int, force_order : bool, double_constraint : bool, output_file_serial, output_file_parallel):
    for i in range(number_of_tests):
        ...
        #    sys.exit(1)
        output_file_serial.write(serial_output)
        output_file_parallel.write(parallel_output)
        output_file_serial.write("\n\n")
        output_file_parallel.write("\n\n")
        

def write_input_files(n : int, force_order : bool, double_constraint : bool):
